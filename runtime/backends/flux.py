"""FLUX-family adapter (FLUX Dev, FLUX Schnell).

FLUX runs flow-matching schedules and low guidance instead of SDXL-style
CFG; the adapter keeps the canonical ``cfg`` key for shape compatibility
and records the guidance semantics in the node notes.
"""
...
class FluxAdapter(BackendAdapter):
    """FLUX family: flow-matching sampler, low guidance."""
    ...
    def adapt(
        ...
        nodes["guidance"] = f"flux guidance {params.cfg:.1f}"
        nodes["scheduler"] = params.scheduler
